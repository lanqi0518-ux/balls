...
| `AllocationBooster` | ✅ | ✅ (stake / unstake) | ❌ | ✅ ($RPO) |
...
1. Launch `$RPO` on Pons (out-of-band; produces the token address).
2. `AllocationBooster(rpoToken)`.
...
7. Run the keeper (`rpo/keeper`); the first tick seeds the "known assets" baseline so pre-existing Stock Tokens don't accidentally trigger a launch.
