...
The gap: no first-class "primary-market access" UI or contract for the global user who doesn't have a Robinhood account. RPO fills that gap by turning **"Robinhood mints a new Stock Token"** into **"user pre-subscribes → gets pro-rata allocation at the earliest possible on-chain price"**.
...
│   ├── test/RPO.t.sol        12 unit tests, all passing
...
Ran 12 tests for test/RPO.t.sol:RPOTest
...
## $RPO token launch on Pons

$RPO is designed to launch on [Pons v2](https://docs.ponsfamily.com/v2) — Robinhood Chain's leading launchpad — with **SPY as quote asset** for the unique "priced against the S&P 500" narrative.
...
80% of platform fees are auto-routed to open-market $RPO buybacks & burns, creating a deflationary flywheel tied to IPO subscription volume.
...
- **DAO governance** — `$RPO` timelock + governor stubs are not scaffolded.
